 ...
 */
if(!function_exists('dateToTimestampLbs')) {
    function dateToTimestampLbs($date) {
        if(empty($a))
            return false;
        $a = explode(LBS_DATE_DL, $date);
...
if(!function_exists('importLbs')) {
    function importLbs($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassLbs')) {
    function importClassLbs($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), LBS_CODE) !== false) {
					$classFile = preg_replace('/'. LBS_CODE. '/i', '', $classFile);
				}
                $path = LBS_CLASSES_DIR. $classFile. '.php';
			}
            return importLbs($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, LBS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], LBS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameLbs')) {
    function toeGetClassNameLbs($class) {
        $className = '';
		if(class_exists($class. strFirstUp(LBS_CODE)))
			$className = $class. strFirstUp(LBS_CODE);
		else if(class_exists(LBS_CLASS_PREFIX. $class))
            $className = LBS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjLbs')) {
    function toeCreateObjLbs($class, $params) {
        $className = toeGetClassNameLbs($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectLbs')) {
    function redirectLbs($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsLbs')) {
	function prepareParamsLbs(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsLbs::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeLbs($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeLbs')) {
	function prepareFieldCodeLbs($string) {
