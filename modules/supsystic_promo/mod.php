<?php
class supsystic_promoLbs extends moduleLbs {
	private $_mainLink = '';
	...
	public function addAdminTab($tabs) {
		$tabs['overview'] = array(
			'label' => __('Overview', LBS_LANG_CODE), 'callback' => array($this, 'getOverviewTabContent'), 'fa_icon' => 'fa-info', 'sort_order' => 5,
		);
		return $tabs;
	}
	...
	public function modifyMainAdminSlug($mainSlug) {
		$firstTimeLookedToPlugin = !installerLbs::isUsed();
	...
	private function _getWelcomMessageMenuData($option, $modifySlug = true) {
		return array_merge($option, array(
			'page_title'	=> __('Welcome to Supsystic Secure', LBS_LANG_CODE),
	...
	private function _getNewAdminMenuSlug($menuSlug) {
		...
		return 'welcome-to-'. frameLbs::_()->getModule('adminmenu')->getMainSlug(). '|return='. $newSlug;
	}
	...
	public function weLoveYou() {
		if(!frameLbs::_()->getModule(implode('', array('l','ic','e','ns','e')))) {
			dispatcherLbs::addFilter('popupEditTabs', array($this, 'addUserExp'));
			dispatcherLbs::addFilter('editPopupMainOptsShowOn', array($this, 'showAdditionalmainAdminShowOnOptions'));
		}
	}
	...
	public function addUserExp($tabs) {
		...
		$tabs['lbsPopupAbTesting'] = array(
			'title' => __('Testing', LBS_LANG_CODE),
			'content' => '<a href="'. $this->_preparePromoLink($url. '?utm_source=plugin&utm_medium=abtesting&utm_campaign=lightbox'). '" target="_blank" class="button button-primary">'. __('Get PRO', LBS_LANG_CODE). '</a><br /><a href="'. $this->_preparePromoLink($url. '?utm_source=plugin&utm_medium=abtesting&utm_campaign=lightbox'). '" target="_blank"><img style="max-width: 100%;" src="'. $modPath. 'img/AB-testing-pro.jpg" /></a>',
			'icon_content' => '<b>A/B</b>',
			'avoid_hide_icon' => true,
			'sort_order' => 55,
		);
		$tabs['lbsPopupLayeredPopup'] = array(
			'title' => __('Layered Style', LBS_LANG_CODE),
	...
	public function checkStatisticStatus(){
		...
		return;
		if(frameLbs::_()->getModule('options')->isEmpty('send_stats')) {	// Enabled by default
			frameLbs::_()->getModule('options')->getModel()->save('send_stats', 1);
		}
		$canSend = (int) frameLbs::_()->getModule('options')->get('send_stats');
		if($canSend) {
	...
	public function getMainLink() {
		if(empty($this->_mainLink)) {
			$this->_mainLink = 'http://supsystic.com/plugins/lightbox-plugin/';
		}
		return $this->_mainLink ;
	}
	public function getContactFormFields() {
		$fields = array(
            'name' => array('label' => __('Your name', LBS_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'text'),
			'email' => array('label' => __('Your email', LBS_LANG_CODE), 'html' => 'email', 'valid' => array('notEmpty', 'email'), 'placeholder' => 'example@mail.com', 'def' => get_bloginfo('admin_email')),
			'website' => array('label' => __('Website', LBS_LANG_CODE), 'html' => 'text', 'placeholder' => 'http://example.com', 'def' => get_bloginfo('url')),
			'subject' => array('label' => __('Subject', LBS_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'text'),
            'category' => array('label' => __('Topic', LBS_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'selectbox', 'options' => array(
				'plugins_options' => __('Plugin options', LBS_LANG_CODE),
				'bug' => __('Report a bug', LBS_LANG_CODE),
				'functionality_request' => __('Require a new functionallity', LBS_LANG_CODE),
				'other' => __('Other', LBS_LANG_CODE),
			)),
			'message' => array('label' => __('Message', LBS_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'textarea', 'placeholder' => __('Hello Supsystic Team!', LBS_LANG_CODE)),
        );
	...
	public function isPro() {
		return frameLbs::_()->getModule('license') ? true : false;
	}
}