<?php
class optionsViewLbs extends viewLbs {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = LBS_DIR. 'readme.txt';
		if(file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if(!empty($readmeContent)) {
				$matchedData = '';
				if(preg_match('/= '. LBS_VERSION. ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} elseif(preg_match('/= '. LBS_VERSION. ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if(isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			frameLbs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
		...
		frameLbs::_()->addJSVar('adminOptionsLbs', 'lbsActiveTab', $activeTab);
		...
		$this->assign('breadcrumbs', frameLbs::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
		$this->assign('mainLink', frameLbs::_()->getModule('supsystic_promo')->getMainLink());
	...
	public function getTabContent() {
		frameLbs::_()->addScript('admin.mainoptions', $this->getModule()->getModPath(). 'js/admin.mainoptions.js');
	...
	public function getSettingsTabContent() {
		frameLbs::_()->addScript('admin.settings', $this->getModule()->getModPath(). 'js/admin.settings.js');
		frameLbs::_()->getModule('templates')->loadJqueryUi();
		
		$options = frameLbs::_()->getModule('options')->getAll();
