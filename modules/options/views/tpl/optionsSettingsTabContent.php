<section class="supsystic-bar">
	<ul class="supsystic-bar-controls">
		<li title="<?php _e('Save all options')?>">
			<button class="button button-primary" id="lbsSettingsSaveBtn" data-toolbar-button>
...
<section>
	<form id="lbsSettingsForm" class="lbsInputsWithDescrForm">
							...
							<?php foreach($optCatData['opts'] as $optKey => $opt) { ?>
								<?php
									...
									if(isset($opt['pro']) && !empty($opt['pro'])) {
										$htmlOpts['attrs'] .= ' class="lbsProOpt"';
									}
								?>
								<tr>
									<th scope="row" class="col-w-30perc">
										...
										<?php if(isset($opt['pro']) && !empty($opt['pro'])) { ?>
											<span class="lbsProOptMiniLabel">
												<a href="<?php echo $opt['pro']?>" target="_blank">
													<?php _e('PRO option', LBS_LANG_CODE)?>
												</a>
											</span>
										<?php }?>
									</th>
									...
									<td class="col-w-1perc">
										<?php echo htmlLbs::$htmlType('opt_values['. $optKey. ']', $htmlOpts)?>
									</td>
									<td class="col-w-60perc">
										<div id="lbsFormOptDetails_<?php echo $optKey?>" class="lbsOptDetailsShell">
										<?php switch($optKey) {
		...
		<?php echo htmlLbs::hidden('mod', array('value' => 'options'))?>
		<?php echo htmlLbs::hidden('action', array('value' => 'saveGroup'))?>
	</form>
</section>