<?php
class admin_navLbs extends moduleLbs {
	public function getBreadcrumbsList() {
		$res = array(
			array('label' => LBS_WP_PLUGIN_NAME, 'url' => frameLbs::_()->getModule('adminmenu')->getMainLink()),
		);
		// Try to get current tab breadcrumb
		$activeTab = frameLbs::_()->getModule('options')->getActiveTab();
		if(!empty($activeTab) && $activeTab != 'main_page') {
			...
			if(!empty($tabs) && isset($tabs[ $activeTab ])) {
				...
				if($activeTab == 'statistics') {
					$statTabs = frameLbs::_()->getModule('statistics')->getStatTabs();
					$currentStatTab = frameLbs::_()->getModule('statistics')->getCurrentStatTab();
