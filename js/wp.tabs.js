	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('lbsWpTabs')) {
					$this.addClass('lbsWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.lbsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.lbsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.lbsTabContent');
					allTabsContent.hide();
