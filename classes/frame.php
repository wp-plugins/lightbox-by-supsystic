...
class frameLbs {
    ...
    public function __construct() {
        $this->_res = toeCreateObjLbs('response', array());
        
    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameLbs();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsLbs::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassLbs($code. strFirstUp(LBS_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameLbs($code);
    ...
    public function init() {
		...
		dispatcherLbs::doAction('afterModulesInit');
		
		modInstallerLbs::checkActivationMessages();
        ...
        register_activation_hook(  LBS_DIR. DS. LBS_MAIN_FILE, array('utilsLbs', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(LBS_DIR. DS. LBS_MAIN_FILE, array('utilsLbs', 'deletePlugin'));
        
		add_action('init', array($this, 'connectLang'));
        //$operationTime = microtime(true) - $startTime;
    }
	public function connectLang() {
		load_plugin_textdomain(LBS_LANG_CODE, false, LBS_PLUG_NAME. '/lang');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', LBS_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[LBS_METHODS])
                    && !empty($permissions[LBS_METHODS])
                ) {
                    foreach($permissions[LBS_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[LBS_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[LBS_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameLbs::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ LBS_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ LBS_METHODS ][ $action ]))
                            || (!is_array($permissions[ LBS_METHODS ][ $action ]) && $permissions[LBS_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[LBS_USERLEVELS])
                    && !empty($permissions[LBS_USERLEVELS])
                ) {
                    //$currentUserPosition = frameLbs::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = LBS_ADMIN;
					}
                    foreach($permissions[LBS_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = LBS_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = LBS_TABLES_DIR) {
        importClassLbs('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableLbs::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameLbs::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriLbs::_($src);
    ...
    public function isTplEditor() {
        $tplEditor = reqLbs::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqLbs::getVar('page');
		if(is_admin() && strpos($page, frameLbs::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(LBS_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(LBS_CODE. '_plugin_activation_errors');
	}
}
