...
class langLbs {
	...
	static public function extract($d = array('dir' => '', 'langLbs' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langLbs']) && !empty($d['langLbs']))
			$langDirPath = LBS_LANG_DIR. $d['langLbs']. DS;
		else
			$langDirPath = LBS_LANG_DIR. LBS_WPLANG. DS;
