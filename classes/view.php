<?php
abstract class viewLbs extends baseObjectLbs {
    /* 
     * @deprecated
     */
    protected $_tpl = LBS_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$code = $this->_code;
		$parentModule = frameLbs::_()->getModule( $this->_code );
		$plTemplate = frameLbs::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !frameLbs::_()->getModule($plTemplate))
			$plTemplate = '';
		if(file_exists(utilsLbs::getCurrentWPThemeDir(). 'lbs'. DS. $code. DS. $tpl. '.php')) {
            $path = utilsLbs::getCurrentWPThemeDir(). 'lbs'. DS. $code. DS. $tpl. '.php';
        } elseif($plTemplate && file_exists(frameLbs::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php')) {
			$path = frameLbs::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php';
	...
	public function getModule() {
		return frameLbs::_()->getModule( $this->_code );
	}
	public function getModel($code = '') {
		return frameLbs::_()->getModule( $this->_code )->getController()->getModel($code);
	}
