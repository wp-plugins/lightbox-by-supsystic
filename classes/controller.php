...
abstract class controllerLbs {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameLbs::_()->getModule( $this->getCode() );
		$className = '';
		if(importLbs($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameLbs($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importLbs($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameLbs($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(LBS_ADMIN),
				'remove' => array(LBS_ADMIN),
				'restore' => LBS_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - LBS_METHODS or LBS_USERLEVELS
	 */
	public function getPermissions() {
		return array();
	}
	public function getModule() {
		return frameLbs::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqLbs::getVar('page');
		$rowsLimit = (int) reqLbs::getVar('rows');
		$orderBy = reqLbs::getVar('sidx');
		$sortOrder = reqLbs::getVar('sord');

		// Our custom search
		$search = reqLbs::getVar('search');
		...
		// jqGrid search
		$isSearch = reqLbs::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqLbs::getVar('searchField'));
			$searchString = trim(reqLbs::getVar('searchString'));
		...
		$res = dispatcherLbs::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();
	}
	public function removeGroup() {
		$res = new responseLbs();
		if($this->getModel()->removeGroup(reqLbs::getVar('listIds', 'post'))) {
