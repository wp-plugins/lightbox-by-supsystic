...
class fieldAdapterLbs {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterLbs', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsLbs'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsLbs::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsLbs::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameLbs::_()->getModule('products')->getCategories();
        $brands = frameLbs::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', LBS_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', LBS_LANG_CODE). '</b><br />
                '. __('Destination', LBS_LANG_CODE). ':'. htmlLbs::checkboxlist('params[dest]', array('optionsLbs' => $listOfDest)). '<br />
                '. __('Country', LBS_LANG_CODE). ':'. htmlLbs::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', LBS_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', LBS_LANG_CODE) : htmlLbs::checkboxlist('params[categories][]', array('optionsLbs' => $cOptions))). '<br />
                    <b>'. __('Brands', LBS_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', LBS_LANG_CODE) : htmlLbs::checkboxlist('params[brands][]', array('optionsLbs' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', LBS_LANG_CODE).': '. htmlLbs::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', LBS_LANG_CODE).': '. htmlLbs::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', LBS_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameLbs::_()->getModule('optionsLbs')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlLbs::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', LBS_LANG_CODE).':<br />';
                $output .= htmlLbs::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', LBS_LANG_CODE).':<br />';
                $output .= htmlLbs::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsLbs']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsLbs'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', LBS_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsLbs'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameLbs::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameLbs::_()->getTable('states')
				->leftJoin( frameLbs::_()->getTable('countries'), 'country_id' )
