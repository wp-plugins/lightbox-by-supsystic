...
class dbLbs {
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, LBS_DB_PREF, $wpdb->prefix. LBS_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampLbs($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, LBS_DATE_DL)) return dateToTimestampLbs($date);
        $arr = explode('-', $date);
        return dateToTimestampLbs($arr[2]. LBS_DATE_DL. $arr[1]. LBS_DATE_DL. $arr[0]);
    }
