...
class responseLbs {
    ...
    public function ajaxExec($forceAjax = false) {
        $isAjax = $this->isAjax();
        $redirect = reqLbs::getVar('redirect');
        if(count($this->errors) > 0)
            $this->error = true;
        if($isAjax || $forceAjax)
            exit( json_encode($this) );
        /*if($redirect)
            redirectLbs($redirect);*/
        return $this;
    }
	public function mainRedirect($redirectUrl = '') {
		$redirectUrl = empty($redirectUrl) ? LBS_SITE_URL : $redirectUrl;
		$redirectData = array();
		if(!empty($this->errors)) {
			$redirectData['lbsErrors'] = $this->errors;
		}
		if(!empty($this->messages)) {
			$redirectData['lbsMsgs'] = $this->messages;
		}
        return redirectLbs($redirectUrl. (strpos($redirectUrl, '?') ? '&' : '?'). http_build_query($redirectData));
	}
    ...
    /**
     * Alias for responseLbs::addError, @see addError method
     */
